...
LABEL version="v0.0.20"
LABEL release="pipetools"
LABEL maintainer="marcinbojko"
SHELL ["/bin/ash", "-eo", "pipefail", "-c"]
# additions
COPY --from=hashicorp/terraform:1.1.9 /bin/terraform /bin/terraform
COPY --from=aquasec/tfsec:v1.19.1 /usr/bin/tfsec /bin/tfsec
COPY --from=anchore/syft:v0.45.1 /syft /bin/syft
COPY --from=hadolint/hadolint:v2.10.0 /bin/hadolint /bin/hadolint
COPY --from=ghcr.io/terraform-linters/tflint:v0.36.1 /usr/local/bin/tflint /bin/tflint
