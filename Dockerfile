...
# additions
COPY --from=hashicorp/terraform:1.2.1 /bin/terraform /bin/terraform
COPY --from=aquasec/tfsec:v1.21.1 /usr/bin/tfsec /bin/tfsec
COPY --from=anchore/syft:v0.46.3 /syft /bin/syft
COPY --from=hadolint/hadolint:v2.10.0 /bin/hadolint /bin/hadolint
COPY --from=ghcr.io/terraform-linters/tflint:v0.36.2 /usr/local/bin/tflint /bin/tflint
