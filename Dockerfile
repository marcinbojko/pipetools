...
COPY --from=hashicorp/terraform:1.3.3 /bin/terraform /bin/terraform
COPY --from=aquasec/tfsec:v1.28.0 /usr/bin/tfsec /bin/tfsec
COPY --from=anchore/syft:v0.59.0 /syft /bin/syft
COPY --from=hadolint/hadolint:v2.10.0 /bin/hadolint /bin/hadolint
COPY --from=ghcr.io/terraform-linters/tflint:v0.42.1 /usr/local/bin/tflint /bin/tflint
COPY --from=datree/datree:1.6.42 /datree /bin/datree
