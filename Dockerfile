...
# additions
COPY --from=hashicorp/terraform:1.15.2 /bin/terraform /bin/terraform
COPY --from=ghcr.io/opentofu/opentofu:1.11.6-minimal /usr/local/bin/tofu /bin/tofu
...
COPY --from=infisical/cli:0.43.84 /bin/infisical /bin/infisical
