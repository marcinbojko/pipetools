...
COPY --from=hashicorp/terraform:1.3.6 /bin/terraform /bin/terraform
COPY --from=aquasec/tfsec:v1.28.1 /usr/bin/tfsec /bin/tfsec
COPY --from=anchore/syft:v0.62.3 /syft /bin/syft
COPY --from=hadolint/hadolint:v2.12.0 /bin/hadolint /bin/hadolint
COPY --from=ghcr.io/terraform-linters/tflint:v0.43.0 /usr/local/bin/tflint /bin/tflint
COPY --from=datree/datree:1.8.8 /datree /bin/datree
