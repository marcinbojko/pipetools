...
# additions
COPY --from=hashicorp/terraform:1.13.0 /bin/terraform /bin/terraform
COPY --from=anchore/syft:v1.31.0 /syft /bin/syft
COPY --from=hadolint/hadolint:v2.12.0 /bin/hadolint /bin/hadolint
COPY --from=ghcr.io/terraform-linters/tflint:v0.58.1 /usr/local/bin/tflint /bin/tflint
COPY --from=axiomhq/cli:0.14.1 /usr/bin/axiom /bin/axiom
COPY --from=infisical/cli:0.41.2 /bin/infisical /bin/infisical
